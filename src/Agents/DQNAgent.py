...
class DQNNetwork(nn.Module):
    """Deep Q-Network architecture with optional regularization."""
...
class DQNAgent(BaseAgent):
    """Deep Q-Network agent for Snake game with advanced features."""
    ...
    def __init__(self, env: SnakeLogic):
        ...
        # LOAD CONFIGURATION
        # ============================================================================
        with open("Hyperparameters/DQN.yaml") as f:
        ...
        self.state_size = env_cfg["state_dim"]
        ...
        self.policy_net = DQNNetwork(
            self.state_size, hidden_layers, self.action_size, dropout_rate=dropout
        ).to(self.device)
    ...
    def _setup_logging(self):
        ...
        if os.path.exists(checkpoint_path):
            ...
            # Fresh training, new log file
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            log_file = os.path.join(self.logs_dir, f"run_{timestamp}.log")

        self.logger = logging.getLogger(f"DQNAgent_{os.path.basename(log_file)}")
        ...
        self.logger.info("===== DQN TRAINING SESSION STARTED =====")
        self.logger.info(f"Device: {self.device}")
        self.logger.info(f"Network: {self.params['network']['hidden_layers']}")
        self.logger.info(f"Learning Rate: {self.learning_rate}")

    def extract_state(self):
        """Extract rich state features from the environment."""
        env = self.env
        features = []

        head = env.snake[0]
        hx, hy = env.index_to_cell(head)
        fx, fy = env.index_to_cell(env.food)

        # Current direction one-hot (4)
        for d in env.get_actions():
            features.append(1.0 if env.direction == d else 0.0)

        # Danger scans: straight, left, right (6)
        free_set = env.free_index
        
        def scan_dir(dx, dy):
            x, y = hx, hy
            for dist in range(1, env.size + 1):
                x += dx
                y += dy
                if x < 0 or x >= env.width or y < 0 or y >= env.height:
                    return dist, 1
                if env.cell_to_index(x, y) not in free_set:
                    return dist, 2
            return env.size, 0

        for rel_dir in [env.relative_to_absolute(rd) for rd in env.get_actions(relative=True)]:
            dx, dy = env._DIRECTIONS[rel_dir]
            dist, danger_type = scan_dir(dx, dy)
            features.append(dist / max(env.width, env.height))
            features.append(float(danger_type))
        ...
        features.append((fx - hx) / env.width)
        features.append((fy - hy) / env.height)
        ...
        def has_path(start, goal):
            ...
            while q:
                ...
                if c == goal:
                    return True
                x, y = env.index_to_cell(c)
                for dx, dy in env._DIRECTIONS.values():
                    nx, ny = x + dx, y + dy
                    if 0 <= nx < env.width and 0 <= ny < env.height:
                        nc = env.cell_to_index(nx, ny)
                        if nc in free_set and nc not in seen:
                            seen.add(nc)
                            q.append(nc)
            return False

        features.append(1.0 if has_path(head, env.food) else 0.0)
        features.append(1.0 if has_path(head, env.snake[-1]) else 0.0)

        # Flood fill (1)
        q, seen = deque([head]), {head}
        while q:
            c = q.popleft()
            x, y = env.index_to_cell(c)
            for dx, dy in env._DIRECTIONS.values():
                ...
                if 0 <= nx < env.width and 0 <= ny < env.height:
                    ...
                    if nc in free_set and nc not in seen:
                        seen.add(nc)
                        q.append(nc)
        features.append(len(seen) / env.size)

        # Snake length (1)
        features.append(len(env.snake) / env.size)
