...
class CNNNetwork(nn.Module):
    ...
    def __init__(self, board_size, conv_filters, kernel_sizes, fc_layers, output_size, dropout_rate=0.0):
        ...
        conv_layers = []
        in_channels = 4  # empty, food, head, body gradient
    ...
    def forward(self, x):
        ...
        # channels: empty, food, head, body gradient
...
class CNNDDQNAgent(BaseAgent):
    """CNN-based Double Deep Q-Network agent for Snake game.
    
    DDQN improvement over DQN: decouples action *selection* from action *evaluation*
    during the TD target computation. The policy network selects the best next action,
    and the target network evaluates its value. This reduces the overestimation bias
    inherent in standard DQN.
    """
    ...
    def __init__(self, env: SnakeLogic):
        ...
        # LOAD CONFIGURATION
        # ============================================================================
        with open("E:\coding\Git_Repos\snake-ai\src\Hyperparameters\CNNDDQN.yaml") as f:
        ...
        self.board_size = env_cfg["board_size"]
        ...
        conv_filters = net_cfg["conv_filters"]
        kernel_sizes = net_cfg["kernel_sizes"]
        fc_layers = net_cfg["fc_layers"]
    ...
    def _setup_logging(self):
        ...
        self.logger = logging.getLogger(f"CNNDDQNAgent_{os.path.basename(log_file)}")
        ...
        self.logger.info("===== CNN DDQN TRAINING SESSION STARTED =====")
    ...
    def extract_state(self):
        """Encode the board into 4 channels.

        Channel 0: empty cells   (1.0 where empty, 0.0 elsewhere)
        Channel 1: food          (1.0 at food cell)
        Channel 2: head          (1.0 at head cell)
        Channel 3: body gradient (0.0 = free, 0.1 = tail, 1.0 = neck)
                                  neck stays occupied longest,
                                  tail leaves on the very next step.
        """
        board_array = np.array(self.env.get_board(), dtype=np.int32)

        # Channels 0-2: one-hot for empty (0), food (1), head (2)
        num_categories = 3
        one_hot = (np.arange(num_categories)[:, None, None] == board_array[None]).astype(np.float32)

        # Channel 3: body gradient
        # snake deque is ordered: head (i=0), neck (i=1), ..., tail (i=body_length)
        gradient_channel = np.zeros((self.board_size, self.board_size), dtype=np.float32)

        snake = self.env.snake
        body_length = len(snake) - 1  # number of body cells, excluding head

        if body_length > 0:
            # max(body_length - 1, 1) ensures tail is always exactly 0.1
            # without this, tail = 0.1 + 0.9 * (1/body_length) which never hits 0.1
            denom = max(body_length - 1, 1)
            for i, cell in enumerate(snake):
                if i == 0:
                    continue  # skip head, already encoded in channel 2
                row = cell // self.board_size
                col = cell % self.board_size
                # neck (i=1) -> 1.0, tail (i=body_length) -> exactly 0.1
                gradient_channel[row][col] = 0.1 + 0.9 * (1.0 - ((i - 1) / denom))

        # Final shape: (4, board_size, board_size)
        return np.concatenate([one_hot, gradient_channel[None]], axis=0)
    ...
    def learn(self):
        ...
        DDQN: policy_net selects the action, target_net evaluates it.
